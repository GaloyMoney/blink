...
export const listIdentities = async (): Promise<IdentityPhone[] | KratosError> => {
  try {
    ...
    // const res = await kratosAdmin.adminListIdentities(perPage, undefined)
    let page: number | undefined = 0

    while (hasNext) {
      // Note: this call is paginated, only return 250 records by default
      ...
      // console.log({ prevPage, page, link: res.headers.link })
      hasNext = page !== undefined
    }
    ...
    console.log({ uniqueIdentities: uniqueIdentities.length, totalCount })
