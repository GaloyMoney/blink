...
export const migrateLnPaymentsFromLnd = async (lnd): Promise<true | ApplicationError> => {
  const paymentHashes = await LedgerService().listPaymentHashes()
  for await (const paymentHash of paymentHashes) {
    console.log("HERE 0:", paymentHash)
    ...
    console.log("HERE 1:", lnPaymentExists)
