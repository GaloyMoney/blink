...
export const addSettledTransaction = async (args: {
  ...
  const wallet = await WalletsRepository().findByAddress(args.address)
  if (wallet instanceof Error) return wallet

  return LockService().lockOnChainTxHashAndVout(
    { txHash: args.txId, vout: args.vout },
    () => lockedAddSettledTransaction({ wallet, ...args }),
  )
}

const lockedAddSettledTransaction = async ({
  wallet,
  txId: txHash,
  vout,
  satoshis: amount,
