...
it('init-order', async () => {
  const { btcAmount, buyOrSell } = FtxBrokerWallet.isOrderNeeded({ usdLiability: 100, usdExposure: 0, btcPrice: 10000 })
...
it('calculate hedging amount when under exposed', async () => {
  ...
  const { btcAmount, buyOrSell } = FtxBrokerWallet.isOrderNeeded({ usdLiability: 1000, usdExposure: 500, btcPrice: 10000 })
  expect(buyOrSell).toBe("sell")
...
it('calculate hedging amount when over exposed', async () => {
  ...
  const { btcAmount, buyOrSell } = FtxBrokerWallet.isOrderNeeded({ usdLiability: 1000, usdExposure: 2000, btcPrice: 10000 })
  expect(buyOrSell).toBe("buy")
  expect(btcAmount).toBe(0.1)
})

it('calculate hedging when no rebalance is needed', async () => {
  const { btcAmount, buyOrSell } = FtxBrokerWallet.isOrderNeeded({ usdLiability: 1000, usdExposure: 1000, btcPrice: 10000 })
...
it('test prod hedging', async () => { // 
  const { btcAmount, buyOrSell } = FtxBrokerWallet.isOrderNeeded({ usdLiability: 40.31493016, usdExposure: 92.136, btcPrice: 11517 })
  expect(buyOrSell).toBe("buy")
})

// "updatedUsdLiability":40.31493016,"updatedUsdExposure":41.4612,"btcPrice":11517
it('test prod hedging', async () => { // 
  const { btcAmount, buyOrSell } = FtxBrokerWallet.isOrderNeeded({ usdLiability: 40.31493016, usdExposure: 41.4612, btcPrice: 11517 })
  expect(buyOrSell).toBeNull()
})


it('test init account', async () => { // "leverage":null,"collateral":0,"btcPrice":11378,
  const { btcAmount, depositOrWithdraw } = FtxBrokerWallet.isRebalanceNeeded({ usdLiability: 0, btcPrice: 10000, usdCollateral: 0})
  expect(depositOrWithdraw).toBe(null)
})

it('test first deposit', async () => { // "leverage":null,"collateral":0,"btcPrice":11378,
  const { btcAmount, depositOrWithdraw } = FtxBrokerWallet.isRebalanceNeeded({ usdLiability: 1000, btcPrice: 10000, usdCollateral: 0})
...
it('isRebalanceNeeded test over leverage', async () => {
  const { btcAmount, depositOrWithdraw } = FtxBrokerWallet.isRebalanceNeeded({ usdLiability: 1000, btcPrice: 10000, usdCollateral: 200 })
  expect(depositOrWithdraw).toBe("deposit")
...
it('isRebalanceNeeded test under leverage', async () => {
  const { btcAmount, depositOrWithdraw } = FtxBrokerWallet.isRebalanceNeeded({ usdLiability: 1000, btcPrice: 10000, usdCollateral: 900})
...
it('isRebalanceNeeded test outrageously under leverage', async () => {
  const { btcAmount, depositOrWithdraw } = FtxBrokerWallet.isRebalanceNeeded({ usdLiability: 1000, btcPrice: 10000, usdCollateral: 2800 })
  expect(depositOrWithdraw).toBe("withdraw")
...
it('isRebalanceNeeded test no action', async () => {
  const { btcAmount, depositOrWithdraw } = FtxBrokerWallet.isRebalanceNeeded({ usdLiability: 1000, btcPrice: 10000, usdCollateral: 500 })
