  ...
  fields: () => ({
    totalLimit: {
      ...
      resolve: async (source) => {
        ...
        const limitAmount = await getAccountLimitsFromConfig({
          level: account.level,
          limitType,
        })
        if (limitAmount instanceof Error) throw mapError(limitAmount)

        return normalizePaymentAmount(limitAmount).amount
      },
    },
    remainingLimit: {
      type: CentAmount,
