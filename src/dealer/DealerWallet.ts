...
export class DealerWallet extends OnChainMixin(UserWallet) {
    ...
    async executeOrder({ buyOrSell, btcAmount }) {
        ...
        const logOrder = this.logger.child({ symbol: this.symbol, orderType, buyOrSell, btcAmount })
        ...
        try {
            order = await this.exchange.createOrder(this.symbol, orderType, buyOrSell, btcAmount)
        } catch (err) {
