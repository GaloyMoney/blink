...
it('calculate hedging when no rebalance is needed', async () => {
  const { btcAmount, buyOrSell } = FtxDealerWallet.isOrderNeeded({ usdLiability: 1000, usdExposure: 1000, btcPrice: 10000 })
...
it('test init account', async () => { // "leverage":null,"collateral":0,"btcPrice":11378,
  const { btcAmount, depositOrWithdraw } = FtxDealerWallet.isRebalanceNeeded({ usdLiability: 0, btcPrice: 10000, usdCollateral: 0})
...
it('isRebalanceNeeded test no action', async () => {
  const { btcAmount, depositOrWithdraw } = FtxDealerWallet.isRebalanceNeeded({ usdLiability: 1000, btcPrice: 10000, usdCollateral: 500 })
