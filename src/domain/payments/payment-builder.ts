...
export const PaymentBuilder = (builderState: Payment = {} as Payment): PaymentBuilder => {
  ...
  const withCheckedHasBalance = (balance: CurrencyBaseAmount) => {
    const balanceAmount =
      builderState.senderWalletCurrency === WalletCurrency.Btc
        ? paymentAmountFromSats(balance as Satoshis)
        : paymentAmountFromCents(balance as UsdCents)

    const newProps = { balanceAmount, hasEnoughBalance: balance >= balanceAmount.amount }
    return PaymentBuilder({ ...builderState, ...newProps })
  }
