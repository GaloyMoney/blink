...
def build_workspace_node_modules_impl(ctx: AnalysisContext) -> list[DefaultInfo]:
...
build_workspace_node_modules = rule(
...
def migrate_mongo_impl(ctx: AnalysisContext) -> list[[
    DefaultInfo,
    RunInfo,
    ExternalRunnerTestInfo,
]]:
    args = cmd_args()
    args.add(ctx.attrs.subcmd)
    args.add("-f")
    args.add(ctx.attrs.config_file)

    return _npm_test_impl(
        ctx,
        ctx.attrs.migrate_mongo[RunInfo],
        args,
        "migrate-mongo",
    )

_migrate_mongo = rule(
    impl = migrate_mongo_impl,
    attrs = {
        "srcs": attrs.list(
            attrs.source(),
            default = [],
        ...
        "migrate_mongo": attrs.dep(
            providers = [RunInfo],
            doc = """migrate-mongo dependency.""",
        ),
        ...
        "env": attrs.dict(
            ...
            sorted = False,
            default = {},
            doc = """Set environment variables for this rule's invocation of migrate-mongo. The environment
            variable values may include macros which are expanded.""",
        ),
        "labels": attrs.list(
            attrs.string(),
...
def migrate_mongo(
        migrate_mongo_bin = "migrate-mongo",
        node_modules = ":node_modules",
        visibility = ["PUBLIC"],
        **kwargs):
    ...
    _migrate_mongo(
        migrate_mongo = ":{}".format(migrate_mongo_bin),
        node_modules = node_modules,
        visibility = visibility,
        **kwargs,
    )
