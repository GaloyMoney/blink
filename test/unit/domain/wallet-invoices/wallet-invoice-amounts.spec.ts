...
import { BtcPaymentAmount, paymentAmountFromCents, UsdPaymentAmount, WalletCurrency, ZERO_CENTS, ZERO_SATS } from "@domain/shared"
...
describe("WalletInvoiceAmounts", () => {
  ...
  describe("for btc invoice", () => {
    ...
    it ("returns correct amounts", async () => {
      const walletInvoiceAmounts = await WalletInvoiceAmounts({walletInvoice: btcInvoice, receivedBtc, usdFromBtcMidPrice: usdFromBtcMidPriceFn, usdFromBtc: dealer.getCentsFromSatsForImmediateBuy})
      ...
      expect(walletInvoiceAmounts).toEqual(expect.objectContaining({
  ...
  describe("for usd invoice", () => {
    ...
    describe("with cents amount", () => {
      ...
      it ("returns correct amounts", async () => {
        const walletInvoiceAmounts = await WalletInvoiceAmounts({walletInvoice: amountUsdInvoice, receivedBtc, usdFromBtcMidPrice: usdFromBtcMidPriceFn, usdFromBtc: dealer.getCentsFromSatsForImmediateBuy})
  ...
  describe("for usd invoice", () => {
    ...
    describe("with no amount", () => {
      ...
      it ("returns correct amounts", async () => {
        const walletInvoiceAmounts = await WalletInvoiceAmounts({walletInvoice: noAmountUsdInvoice, receivedBtc, usdFromBtcMidPrice: usdFromBtcMidPriceFn, usdFromBtc: dealer.getCentsFromSatsForImmediateBuy})
      
        if (walletInvoiceAmounts instanceof Error) throw walletInvoiceAmounts
        
        const usdToCreditReceiver = await dealer.getCentsFromSatsForImmediateBuy(receivedBtc)
        if (usdToCreditReceiver instanceof Error) throw usdToCreditReceiver
