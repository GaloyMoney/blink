...
describe("IPMetadataValidator", () => {
  describe("isValid", () => {
    it("returns true for empty config", () => {
      const validator = IPMetadataValidator(defaultConfig).isValid(defaultIpInfo)
      expect(validator).toBe(true)
    })

    it("returns true for an allowed country", () => {
      const config = { ...defaultConfig, allowIPCountries: ["US"] }
      const validator = IPMetadataValidator(config).isValid(defaultIpInfo)
      expect(validator).toBe(true)
    })
    ...
    it("returns error for a country not defined in allow-list", () => {
      ...
      const validator = IPMetadataValidator(config).isValid(ipInfo)
    ...
    it("returns error for proxy/vpn", () => {
      ...
      const validator = IPMetadataValidator(defaultConfig).isValid(ipInfo)
    ...
    it("returns error for empty isoCode", () => {
      ...
      let validator = IPMetadataValidator(defaultConfig).isValid(ipInfo)
      expect(validator).toBeInstanceOf(MissingIPMetadataError)

      const ipInfo1 = { ...defaultIpInfo, isoCode: "" }
      validator = IPMetadataValidator(defaultConfig).isValid(ipInfo1)
      expect(validator).toBeInstanceOf(MissingIPMetadataError)
    })
