...
describe("hashApiKey", () => {
  ...
  it("fails if empty key", async () => {
    let hashedKey = await hashApiKey({ key: "", secret: "abc", expireAt })
    expect(hashedKey).toBeInstanceOf(InvalidApiKeyError)
  ...
  it("fails if empty secret", async () => {
    let hashedKey = await hashApiKey({ key: "abc", secret: "", expireAt })
    expect(hashedKey).toBeInstanceOf(InvalidApiKeyError)
  ...
  it("fails if expired", async () => {
    const expireAt = new Date(Date.now() - 1)
    const hashedKey = await hashApiKey({ key: "abc", secret: "abc", expireAt })
    expect(hashedKey).toBeInstanceOf(InvalidExpirationError)
  })
})
