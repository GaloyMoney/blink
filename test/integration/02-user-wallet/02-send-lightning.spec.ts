...
describe("UserWallet - Lightning Pay", () => {
  ...
  it("sends to another Galoy user a push payment", async () => {
    ...
    const finalBalance0 = await getBTCBalance(walletIdA)
    ...
    const finalBalance1 = await getBTCBalance(walletIdB)
    ...
    expect(finalBalance0).toBe(initBalanceA + amountInvoice)
    expect(finalBalance1).toBe(initBalanceB - amountInvoice)
    ...
    const contact0 = userTypeA.contacts.find(
