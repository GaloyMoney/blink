...
describe("UserWallet - Lightning Pay", () => {
  ...
  it("sends to another Galoy user a push payment", async () => {
    ...
    expect(userTransaction0[0]).toHaveProperty("recipient", userWallet1.user.username)
    ...
    expect(userTransaction1[0]).toHaveProperty("recipient", userWallet0.user.username)
  ...
  it("filters spam from send to another Galoy user as push payment", async () => {
    ...
    expect(transaction0Below).toHaveProperty("recipient", userWallet1.user.username)
    expect(transaction0Below.deprecated).toHaveProperty(
      "description",
      `from ${userWallet1.user.username}`,
    )
    expect(transaction1Below).toHaveProperty("recipient", userWallet0.user.username)
    ...
    expect(transaction0Above).toHaveProperty("recipient", userWallet1.user.username)
    expect(transaction0Above.deprecated).toHaveProperty(
      "description",
      memoSpamAboveThreshold,
    )
    expect(transaction1Above).toHaveProperty("recipient", userWallet0.user.username)
